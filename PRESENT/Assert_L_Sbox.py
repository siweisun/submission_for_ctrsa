...
class Assert_L_Sbox(object):
    ...
    def build_constrs(self, inVec, outVec, r, p):
        ...
        for i in range(2 ** self.__dim):
            ...
            if not self.__genInt.getTrails()[x]:
                s += 'ASSERT %s /= %s;\n' % ( 
                        '@'.join(inVec), 
...
def main():
    ...
    a = Assert_L_Sbox('sbox', sbox, 4, 2, 16)
